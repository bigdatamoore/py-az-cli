...
def show(resource_group, partner_topic_name, name, include_full_endpoint_url=None, include_static_delivery_attribute_secret=None):
    params = get_params(locals())   
    command = "az eventgrid partner topic event-subscription show " + params
    print(command)
...
def delete(resource_group, partner_topic_name, name, yes=None):
    params = get_params(locals())   
    command = "az eventgrid partner topic event-subscription delete " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def list(resource_group, partner_topic_name, odata_query=None):
    params = get_params(locals())   
    command = "az eventgrid partner topic event-subscription list " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
