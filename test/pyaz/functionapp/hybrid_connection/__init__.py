...
def list(name, resource_group, slot=None):
    ...
    command = "az functionapp hybrid-connection list " + params
    print(command)
...
def add(name, resource_group, namespace, hybrid_connection, slot=None):
    params = get_params(locals())   
    command = "az functionapp hybrid-connection add " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def remove(resource_group, name, namespace, hybrid_connection, slot=None):
    params = get_params(locals())   
    command = "az functionapp hybrid-connection remove " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
