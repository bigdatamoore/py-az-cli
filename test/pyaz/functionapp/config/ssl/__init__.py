...
def upload(resource_group, name, certificate_password, certificate_file, slot=None):
    ...
    command = "az functionapp config ssl upload " + params
    print(command)
...
def list(resource_group):
    params = get_params(locals())   
    command = "az functionapp config ssl list " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def show(resource_group, certificate_name):
    params = get_params(locals())   
    command = "az functionapp config ssl show " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def bind(resource_group, name, certificate_thumbprint, ssl_type, slot=None):
    params = get_params(locals())   
    command = "az functionapp config ssl bind " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def unbind(resource_group, name, certificate_thumbprint, slot=None):
    params = get_params(locals())   
    command = "az functionapp config ssl unbind " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def delete(resource_group, certificate_thumbprint):
    params = get_params(locals())   
    command = "az functionapp config ssl delete " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def import_(resource_group, name, key_vault, key_vault_certificate_name):
    params = get_params(locals())   
    command = "az functionapp config ssl import " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def create(resource_group, name, hostname, slot=None):
    params = get_params(locals())   
    command = "az functionapp config ssl create " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
