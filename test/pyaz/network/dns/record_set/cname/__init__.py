...
def show(resource_group, zone_name, name, record_type=None):
    ...
    command = "az network dns record-set cname show " + params
    print(command)
...
def delete(resource_group, zone_name, name, record_type=None, if_match=None, yes=None):
    params = get_params(locals())   
    command = "az network dns record-set cname delete " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def list(resource_group, zone_name, record_type=None):
    params = get_params(locals())   
    command = "az network dns record-set cname list " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def create(resource_group, zone_name, name, record_set_type=None, metadata=None, if_match=None, if_none_match=None, ttl=None, target_resource=None):
    params = get_params(locals())   
    command = "az network dns record-set cname create " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def set_record(resource_group, zone_name, record_set_name, cname, ttl=None, if_none_match=None):
    params = get_params(locals())   
    command = "az network dns record-set cname set-record " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def remove_record(resource_group, zone_name, record_set_name, cname, keep_empty_record_set=None):
    params = get_params(locals())   
    command = "az network dns record-set cname remove-record " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
