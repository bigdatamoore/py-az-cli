...
def show(resource_group, zone_name, name, record_type=None):
    ...
    command = "az network dns record-set txt show " + params
    print(command)
...
def delete(resource_group, zone_name, name, record_type=None, if_match=None, yes=None):
    params = get_params(locals())   
    command = "az network dns record-set txt delete " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def list(resource_group, zone_name, record_type=None):
    params = get_params(locals())   
    command = "az network dns record-set txt list " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def create(resource_group, zone_name, name, record_set_type=None, metadata=None, if_match=None, if_none_match=None, ttl=None, target_resource=None):
    params = get_params(locals())   
    command = "az network dns record-set txt create " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def add_record(resource_group, zone_name, record_set_name, value, if_none_match=None):
    params = get_params(locals())   
    command = "az network dns record-set txt add-record " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def remove_record(resource_group, zone_name, record_set_name, value, keep_empty_record_set=None):
    params = get_params(locals())   
    command = "az network dns record-set txt remove-record " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def update(resource_group, zone_name, name, record_type=None, if_match=None, if_none_match=None, metadata=None, target_resource=None, set=None, add=None, remove=None, force_string=None):
    params = get_params(locals())   
    command = "az network dns record-set txt update " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
