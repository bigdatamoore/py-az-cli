...
def show(resource_group, zone_name, record_type=None, name):
    ...
    command = "az network private-dns record-set mx show " + params
    print(command)
...
def delete(resource_group, zone_name, record_type=None, name, if_match=None, yes=None):
    params = get_params(locals())   
    command = "az network private-dns record-set mx delete " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def list(resource_group, zone_name, record_type=None):
    params = get_params(locals())   
    command = "az network private-dns record-set mx list " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def create(resource_group, zone_name, name, record_type=None, metadata=None, ttl=None):
    params = get_params(locals())   
    command = "az network private-dns record-set mx create " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def add_record(resource_group, zone_name, record_set_name, preference, exchange):
    params = get_params(locals())   
    command = "az network private-dns record-set mx add-record " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def remove_record(resource_group, zone_name, record_set_name, preference, exchange, keep_empty_record_set=None):
    params = get_params(locals())   
    command = "az network private-dns record-set mx remove-record " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def update(resource_group, zone_name, record_type=None, name, if_match=None, metadata=None, set=None, add=None, remove=None, force_string=None):
    params = get_params(locals())   
    command = "az network private-dns record-set mx update " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
