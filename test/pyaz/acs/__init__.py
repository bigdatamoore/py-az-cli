...
def create(resource_group, deployment_name=None, name, ssh_key_value=None, dns_prefix=None, location=None, admin_username=None, api_version=None, master_profile=None, master_vm_size=None, master_osdisk_size=None, master_count=None, master_vnet_subnet_id=None, master_first_consecutive_static_ip=None, master_storage_profile=None, agent_profiles=None, agent_vm_size=None, agent_osdisk_size=None, agent_count=None, agent_vnet_subnet_id=None, agent_ports=None, agent_storage_profile=None, orchestrator_type=None, orchestrator_version=None, service_principal=None, client_secret=None, tags=None, windows=None, admin_password=None, generate_ssh_keys=None, validate=None, no_wait=None):
    params = get_params(locals())   
    command = "az acs create " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def list_locations():
    params = get_params(locals())   
    command = "az acs list-locations " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def scale(resource_group, name, new_agent_count):
    params = get_params(locals())   
    command = "az acs scale " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
