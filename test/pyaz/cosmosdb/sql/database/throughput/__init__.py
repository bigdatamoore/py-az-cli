...
def show(resource_group, account_name, name):
    ...
    command = "az cosmosdb sql database throughput show " + params
    print(command)
...
def migrate(resource_group, account_name, name, throughput_type):
    params = get_params(locals())   
    command = "az cosmosdb sql database throughput migrate " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
...
def update(resource_group, account_name, name, throughput=None, max_throughput=None):
    params = get_params(locals())   
    command = "az cosmosdb sql database throughput update " + params
    print(command)
    output = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout =  output.stdout.decode("utf-8")
    stderr = output.stderr.decode("utf-8")
